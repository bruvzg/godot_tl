...
LSR::LSR(char prefix, const char *lang, const char *scr, const char *r, UErrorCode &errorCode) :
		language(nullptr), script(nullptr), region(r), regionIndex(indexForRegion(region)) {
...
LSR::LSR(LSR &&other) U_NOEXCEPT : language(other.language), script(other.script), region(other.region), owned(other.owned), regionIndex(other.regionIndex), hashCode(other.hashCode) {
	if (owned != nullptr) {
	...
	if (hashCode == 0) {
		hashCode =
				(ustr_hashCharsN(language, static_cast<int32_t>(uprv_strlen(language))) * 37 +
						ustr_hashCharsN(script, static_cast<int32_t>(uprv_strlen(script)))) *
						37 +
				regionIndex;
	}
	return *this;
